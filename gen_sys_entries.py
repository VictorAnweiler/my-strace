...
def find_args(linux):
    ...
    find = subprocess.Popen(["find"] +
                             [os.path.join(linux, d) for d in
                              "arch/x86 fs include ipc kernel mm net security".split()] + # include tout les fichier dispo dans le repo linux ici
                            ["-name", "*.c", "-print"],
...
def write_output(syscalls_h, types, numbers):
    ...
    for num in sorted(numbers.keys()):
        ...
        print >>out, "  [%d] = {" % (num,)
        print >>out, "    .sysname  = \"%s\"," % (name,)
        print >>out, "    .nb_args = %d," % (len(args,))
        out.write(   "    .args  = {")
...
def main(args):
    if not args:
        print >>sys.stderr, "Usage: %s /path/to/linux (big up linus)" % (sys.argv[0],)
        return 1
    linux_dir = args[0]
    ...
    write_output('syscall_entries.h', syscall_types, syscall_numbers)
